...
const EditProduct = () => {
  ...
  const addVariant = () => {
    setFormData({ ...formData, variants: [...formData.variants, { size: '', price: '', stock: '' }] });
  };
    ...
    <div className="max-w-4xl mx-auto pb-20 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-amber-50/40 text-sm mt-1">Update details for <span className="text-amber-300/80">{formData.name}</span></p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-white/5 border border-amber-300/10 p-6 rounded-[2.5rem] flex flex-col items-center justify-center gap-4">
            <div className="relative h-44 w-44 rounded-[2rem] border-2 border-dashed border-amber-300/20 overflow-hidden bg-black/40">
              ...
              <label htmlFor="edit-img" className="absolute inset-0 bg-black/60 opacity-0 hover:opacity-100 transition flex items-center justify-center cursor-pointer text-white text-xs font-bold">CHANGE IMAGE</label>
            </div>
          ...
          <div className="md:col-span-2 bg-white/5 border border-amber-300/10 p-8 rounded-[2.5rem] space-y-6">
            ...
            <div>
              ...
              <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-black/40 border border-amber-300/10 rounded-2xl px-5 py-4 outline-none text-white appearance-none">
        ...
        {/* Description & Variants - (Same style as AddProduct) */}
        <div className="bg-white/5 border border-amber-300/10 p-8 rounded-[2.5rem]">
           ...
           <textarea rows="4" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-black/40 border border-amber-300/10 rounded-2xl px-5 py-4 outline-none resize-none"></textarea>
        </div>

        {/* Variants Section */}
        <div className="bg-white/5 border border-amber-300/10 p-8 rounded-[2.5rem]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-amber-300">Update Inventory</h3>
            <button type="button" onClick={addVariant} className="text-xs bg-amber-300/10 text-amber-300 px-4 py-2 rounded-xl border border-amber-300/20 flex items-center gap-2 font-bold"><Plus size={16} /> New Size</button>
          </div>
          <div className="space-y-4">
            {formData.variants.map((v, i) => (
              <div key={i} className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-black/20 rounded-2xl border border-amber-300/5 items-end">
                <input placeholder="Size" value={v.size} onChange={e => updateVariant(i, 'size', e.target.value)} className="bg-black/40 border border-amber-300/10 rounded-xl px-4 py-3 outline-none" required />
                <input type="number" placeholder="Price" value={v.price} onChange={e => updateVariant(i, 'price', e.target.value)} className="bg-black/40 border border-amber-300/10 rounded-xl px-4 py-3 outline-none" required />
                <div className="flex gap-2">
                  <input type="number" placeholder="Stock" value={v.stock} onChange={e => updateVariant(i, 'stock', e.target.value)} className="flex-1 bg-black/40 border border-amber-300/10 rounded-xl px-4 py-3 outline-none" required />
                  {formData.variants.length > 1 && <button type="button" onClick={() => removeVariant(i)} className="p-3 text-red-400 hover:bg-red-500/10 rounded-xl"><Trash2 size={20} /></button>}
                </div>
              </div>
            ))}
          </div>
        ...
        <button 
          disabled={loading} 
          className="w-full py-5 bg-amber-300 text-black font-extrabold text-lg rounded-[2rem] hover:bg-amber-200 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
        >
